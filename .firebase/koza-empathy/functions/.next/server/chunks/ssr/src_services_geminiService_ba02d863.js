module.exports=[57803,a=>{"use strict";var b=a.i(99121);let c=`Sen OMNIVERSAL ANLATI TEKİLLİK MERKEZİ'sin (OMNIVERSAL NARRATIVE SINGULARITY CORE).
...
Sadece JSON yazın.`,e=`Siz bir hikaye edit\xf6r\xfcs\xfcn\xfcz. Mevcut bir hikayeyi ve kullanıcının geri bildirimini alıp hikayeyi bu geri bildirime g\xf6re g\xfcncelliyorsunuz.
...
Sadece JSON yazın.`,f=`Siz etkileşimli bir başkalaşım tasarımcısısınız. Kullanıcının deneyimini 3 seviyeli bir "İ\xe7sel G\xfc\xe7 Labirenti" oyununa d\xf6n\xfcşt\xfcr\xfcyorsunuz.
...
Sadece JSON yazın.`,g=`Siz yaratıcı bir isimlendirme uzmanısınız. Verilen hikaye veya oyun i\xe7eriğine ve bağlamına uygun, "KOZA" evrenine yakışır şekilde metaforik, kısa ve etkileyici bir başlık oluşturun.
...
Bağlam/İ\xe7erik: `,h=new Map,i=a=>{try{let b=a.replace(/```json/g,"").replace(/```/g,"").trim(),c=b.indexOf("["),d=b.indexOf("{"),e=-1,f=-1;return -1!==c&&(-1===d||c<d)?(e=c,f=b.lastIndexOf("]")):-1!==d&&(e=d,f=b.lastIndexOf("}")),-1!==e&&-1!==f&&(b=b.substring(e,f+1)),b}catch{return a}},j=a=>new Promise(b=>setTimeout(b,a)),k=async(a,d,e=3)=>{let f,g=b.API_CONFIG.OPENROUTER_API_KEY,k=`${a.substring(0,50)}_${d.substring(0,100)}`,l=h.get(k);if(l&&Date.now()-l.timestamp<3e5)return console.log("📦 Using cached response"),l.data;for(let b=0;b<e;b++)try{let b=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${g}`,"HTTP-Referer":"https://koza-app.vercel.app","X-Title":"KOZA App"},body:JSON.stringify({model:"google/gemma-3-27b-it",messages:[{role:"system",content:c},{role:"user",content:`${a}

User experience: ${d}`}],temperature:.8,max_tokens:8192,response_format:{type:"json_object"}})});if(!b.ok){let a=await b.text();throw Error(`API error ${b.status}: ${a}`)}let e=(await b.json()).choices[0].message.content,f=JSON.parse(i(e));return h.set(k,{data:f,timestamp:Date.now()}),f}catch(a){if(f=a,console.error(`Attempt ${b+1} failed:`,a.message),b<e-1){let a=Math.min(1e3*Math.pow(2,b),5e3);console.log(`Retrying in ${a}ms...`),await j(a)}}throw Error(`Failed after ${e} attempts: ${f.message}`)},l=async a=>{if(!a||a.trim().length<10)throw Error("Lütfen deneyiminizi açıklayan en az 10 karakter girin");return k(d,a)},m=async(a,b)=>{if(!b||b.trim().length<5)throw Error("Lütfen daha detaylı bir geri bildirim sağlayın (en az 5 karakter)");return k(e.replace("{{EXISTING_STORY}}",JSON.stringify(a)).replace("{{USER_FEEDBACK}}",b),b)},n=async a=>{if(!a||a.trim().length<10)throw Error("Lütfen deneyiminizi açıklayan en az 10 karakter girin");return k(f,a)},o=async a=>{try{let b=g+`

Sadece şu JSON formatında yanıt verin: { "title": "Oluşturulan Başlık" }`;return(await k(b,a)).title}catch{return console.error("İsimlendirme başarısız"),"Dönüşüm Hikayesi"}};a.s(["generateContentName",0,o,"generateGame",0,n,"generateStorybook",0,l,"refineStorybook",0,m],57803)}];
